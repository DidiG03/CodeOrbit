...
export const LaserFlow: React.FC<Props> = React.memo(({
  className,
  style,
  wispDensity = 1,
  dpr,
  ...
  useEffect(() => {
    const mount = mountRef.current!;
      const renderer = new THREE.WebGLRenderer({
        ...
        depth: false,
        stencil: false,
        powerPreference: 'default', // Changed from high-performance for better battery/efficiency
    ...
    // Fixed DPR - capped at 1.0 for maximum performance
    const deviceDpr = dpr ?? (window.devicePixelRatio || 1);
    const fixedDpr = Math.min(deviceDpr, 1.0); // Cap at 1.0 for better performance
    ...
    let prevTime = clock.getElapsedTime(); // Initialize from clock
    let fade = 0; // Always start from 0 for consistent fade-in
    ...
    const animate = () => {
      ...
      // Smooth fade-in that only happens once
      if (!hasFadedRef.current) {
        const fadeDur = 1.5; // Slightly longer fade
        fade = Math.min(1, fade + cdt / fadeDur);
        uniforms.uFade.value = fade;
        if (fade >= 1) {
          hasFadedRef.current = true;
          uniforms.uFade.value = 1; // Ensure it stays at 1
        }
      }
